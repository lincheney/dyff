...
fn main_internal() -> Result<ExitCode> {
    ...
    loop {
        ...
        if !markdown && byte_regex!(r"^```diff\s*$".is_match(&stripped)) {
            if let Some(mut hunk) = hunk {
                hunk.print(&mut stdout, &mut tokeniser, line_numbers, merge_markers.as_ref(), style, &args.style)?;
            }
            markdown = true;
            ...
            hunk = Some(Hunk::new());
            // i might not have actual line numbers
            line_numbers = [1, 1];
            continue
        }
        ...
        if unified
            && let Some(merge_markers) = &mut merge_markers
        ...
        if h.is_empty() && let Some(captures) = byte_regex!(r"^rename (?<sign>to|from)[ \t](?<filename>.*\n)".captures(&stripped)) {
        ...
        if markdown && byte_regex!(r"^```\s*$".is_match(&stripped)) {
            markdown = false;
            h.print(&mut stdout, &mut tokeniser, line_numbers, merge_markers.as_ref(), style, &args.style)?;
            hunk = None;
            args.style.print_background(&mut stdout)?;
